...
namespace FitBitAssistantBot
{
    ...
    public class FitBitApiHelper
    {
        ...
        /// <summary>
        /// Get the Acitivity Data
        /// </summary>
        /// <returns>UserActivity Object</returns>
        public async Task<UserActivity> GetUserActivityAsync(DateTime dated)
        {

            string url = Constants.UserActivity.Replace("[date]", dated.ToString("yyyy-MM-dd"));
            string jsonResponse = await GetFitBitDataAsync(url);
           var info = JsonConvert.DeserializeObject<UserActivity>(jsonResponse);
            return info;
